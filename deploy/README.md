...
Для первой проверки на VPS с 1 GB RAM используется
[нативный вариант nginx + PostgreSQL 16 + systemd](native/README.md).
Инструкция Compose ниже относится к альтернативному варианту, не к
одновременно работающему окружению.
...
Удалённые операции выполняются через runner на утверждённом VPS
62.113.109.168; результаты и отчёты хранятся в папке проекта.
