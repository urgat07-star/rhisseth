...
if len(sys.argv) != 3 or sys.argv[1] != rid or sys.argv[2] not in ('inspect', 'deploy', 'validate', 'timer-check'):
...
try:
    ...
    result = ssh(['scp', *options, str(Path(__file__).with_name('GameAccessIdentity.py')), 'root@62.113.109.168:/opt/rhisseth/scripts/automation/GameAccessIdentity.py'])
    if result.returncode:
        raise RuntimeError('Reviewed identity helper transfer failed')
