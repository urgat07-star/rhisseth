...
def main():
    ...
    try:
        ...
        if run(['git', '-C', str(repo), 'status', '--porcelain']):
            raise RuntimeError('Uncommitted code changes; inventory required')
        ...
        audit('Preflight: root, lock, clean Git checkout and free space verified')
        ...
        with tempfile.TemporaryDirectory(prefix='.work-', dir=spool) as temporary:
            ...
            manifest = {'format':1, 'created_utc':now.isoformat(), 'timezone':'Europe/Moscow', 'signature_order':'C', 'signature_timezone':'UTC', 'git_sha':sha, 'postgresql_major':16, 'schema_migrations':schema, 'migrations_sha256':migration_hashes, 'table_signatures':signatures, 'files_sha256':files, 'project_paths':[p.name for p in included], 'excluded':['Git code/runtime','previous backups','logs','automation scripts'], 'unencrypted':True}
