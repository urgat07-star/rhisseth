...
try:
    ...
    payload_name = 'HexMetadataVps.py'
    ...
    result = ssh(['scp', *options, str(Path(__file__).with_name('VerifyHexMetadata.py')), 'root@62.113.109.168:/opt/rhisseth/scripts/automation/VerifyHexMetadata.py'])
    ...
    if operation == 'deploy':
        result = ssh(['scp', *options, str(root / 'temp/hex-metadata.tar'), 'root@62.113.109.168:/opt/rhisseth/temp/hex-metadata.tar'])
    ...
    code = process.wait()
    if code == 0:
        (root / 'reports/hex-metadata-live-files').mkdir(exist_ok=True)
        result = ssh(['scp', '-r', *options, 'root@62.113.109.168:/opt/rhisseth/reports/hex-metadata-live-files/.', str(root / 'reports/hex-metadata-live-files')])
        if result.returncode: raise RuntimeError('Live frontend retrieval failed')
        result = ssh(['scp', *options, 'root@62.113.109.168:/opt/rhisseth/reports/hex-metadata-source.json', str(root / 'reports/hex-metadata-source.json')])
