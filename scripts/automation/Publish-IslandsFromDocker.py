...
log = directory / (now.strftime('%Y%m%d-%H%M%S') + '-islands-local-docker.md')
...
try:
    ...
    with tempfile.TemporaryDirectory(prefix='rhisseth-ssh-', dir='/run') as scratch:
        ...
        if args.operation == 'publish':
            run([*prefix, 'ssh', *options, target,
                 'mkdir -p /opt/rhisseth/staging/islands-map-v1 /opt/rhisseth/repository/scripts/automation'], env=env)
            for name in ('index.html', 'terrain-map-islands-preview-v1.png'):
                run([*prefix, 'scp', *options, str(root / 'reports/previews/islands-v1' / name),
                     target + ':/opt/rhisseth/staging/islands-map-v1/' + name], env=env)
            run([*prefix, 'scp', *options, str(root / 'scripts/automation/Publish-IslandsPreviewVps.py'),
                 target + ':/opt/rhisseth/repository/scripts/automation/Publish-IslandsPreviewVps.py'], env=env)
            run([*prefix, 'ssh', *options, target,
                 'flock -n /opt/rhisseth/.local/islands-publication.lock /opt/rhisseth/venv/bin/python /opt/rhisseth/repository/scripts/automation/Publish-IslandsPreviewVps.py'], env=env, timeout=300)
