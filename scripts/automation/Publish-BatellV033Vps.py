#!/usr/bin/python3
"""Deploy the reviewed v0.3.3 archive without resetting player state."""
...
LOG = LOG_DIR / (STAMP + '-batell-v033-publish.md')
LOG.write_text(f'''# Rhisseth batell v0.3.3 publication

- UTC: {NOW.isoformat()}
- Europe/Moscow: {NOW.astimezone(ZoneInfo('Europe/Moscow')).isoformat()}
- Task: batell-v0.3.3-live-test; operator/controller: Codex
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] != 'publish-v033' or os.geteuid() != 0:
    ...
    stage = ROOT / ('repository.v033-staging-' + STAMP)
    backup = ROOT / 'backups' / ('v033-publish-' + STAMP)
    old_repo = backup / 'repository-before'
    if stage.exists() or backup.exists():
        raise RuntimeError('Publication stage or backup already exists')
    stage.mkdir(mode=0o755)
    digest = hashlib.sha256(ARCHIVE.read_bytes()).hexdigest()
    emit(f'Preflight: reviewed v0.3.3 archive bytes={ARCHIVE.stat().st_size}; SHA256={digest}')
    ...
    required = [stage / 'app/backend/migrate.py', stage / 'data/migrations/035_batell_v032.sql',
                stage / 'app/frontend/index.html', stage / 'docs/releases/batell-v0.3.3.md']
    if any(not item.is_file() for item in required):
        raise RuntimeError('v0.3.3 archive incomplete')
    ...
    emit('Preflight: v0.3.2 database baseline confirmed; v0.3.3 files staged')
    ...
    try:
        ...
        emit(f'Validation: v0.3.3 live; migration 035 remains applied; backup={dump}; '
        ...
        if swapped:
            try:
                run(['systemctl', 'stop', 'rhisseth'])
                stopped = True
                failed = ROOT / ('repository.v033-failed-' + STAMP)
                REPO.rename(failed)
                (backup / 'repository-swapped').rename(REPO)
