#!/usr/bin/python3
"""Deploy the reviewed v0.3.2 archive without resetting player state."""
...
LOG = LOG_DIR / (STAMP + '-batell-v032-publish.md')
LOG.write_text(f'''# Rhisseth batell v0.3.2 publication

- UTC: {NOW.isoformat()}
- Europe/Moscow: {NOW.astimezone(ZoneInfo('Europe/Moscow')).isoformat()}
- Task: batell-v0.3.2-live-test; operator/controller: Codex
- Runner: STU-AUTOMATION-01 / 10.210.52.128
- Target: Rhisseth VDS 62.113.109.168, PostgreSQL rhisseth, rhisseth service
- Action: verified current backup, migration 035, tagged-code deployment, validation
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] != 'publish-v032' or os.geteuid() != 0:
    ...
    stage = ROOT / ('repository.v032-staging-' + STAMP)
    backup = ROOT / 'backups' / ('v032-publish-' + STAMP)
    old_repo = backup / 'repository-before'
    if stage.exists() or backup.exists():
        raise RuntimeError('Publication stage or backup already exists')
    stage.mkdir(mode=0o755)
    digest = hashlib.sha256(ARCHIVE.read_bytes()).hexdigest()
    emit(f'Preflight: reviewed v0.3.2 archive bytes={ARCHIVE.stat().st_size}; SHA256={digest}')
    ...
    required = [stage / 'app/backend/migrate.py', stage / 'data/migrations/035_batell_v032.sql',
                stage / 'app/frontend/index.html', stage / 'docs/releases/batell-v0.3.2.md']
    if any(not item.is_file() for item in required):
        raise RuntimeError('v0.3.2 archive incomplete')
    ...
    emit('Preflight: v0.3.1 database baseline confirmed; v0.3.2 files staged')
    ...
    try:
        ...
        emit(f'Validation: v0.3.2 live; migration 035 applied; backup={dump}; '
        ...
        if swapped:
            try:
                run(['systemctl', 'stop', 'rhisseth'])
                stopped = True
                failed = ROOT / ('repository.v032-failed-' + STAMP)
                REPO.rename(failed)
                (backup / 'repository-swapped').rename(REPO)
