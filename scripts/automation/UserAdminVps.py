...
log = directory / (now.strftime('%Y%m%d-%H%M%S')+'-user-admin-'+operation+'.md')
log.write_text(f'# User administration {operation}\nUTC: {now.isoformat()}\nEurope/Moscow: {now.astimezone(ZoneInfo("Europe/Moscow")).isoformat()}\nTask: rhisseth-user-admin; Controller: Codex via STU-AUTOMATION-01 / 10.210.52.128\nTarget: 62.113.109.168 / rhisseth.ru; action: {operation}\nPassbolt: da44a388-e458-4379-ab4c-c204696804b2\n',encoding='utf-8')
def emit(value):
...
try:
    ...
    if operation=='deploy':
        package=ROOT/'temp/user-admin.tar'
        backup=ROOT/'backups'/('user-admin-'+now.strftime('%Y%m%d-%H%M%S'))
        ...
        allowed={'app/backend/main.py','app/backend/admin_users.py','app/frontend/app.js','app/frontend/index.html','app/frontend/users.html','app/frontend/users.js','data/migrations/004_user_admin.sql','data/migrations/005_user_deletion_audit.sql'}
        ...
        test_db='rhisseth_user_admin_check'
        ...
        try:
            ...
            emit(run([str(ROOT/'venv/bin/python'),str(Path(__file__).with_name('VerifyUserAdmin.py')),str(stage),test_db]))
        ...
        try:
            for path in ('/api/hexes','/admin/users','/api/admin/users','/admin/hexes','/api/start/options'):
                ...
                with urllib.request.urlopen(req,timeout=10) as response:
                    ...
                    if path=='/admin/users':assert b'name="password"' in content and b'name="password_confirm"' in content
        ...
        if release.exists():
            metadata=json.loads(release.read_text());metadata.update(user_admin_deployed_utc=now.isoformat(),user_admin_package_sha256=hashlib.sha256(package.read_bytes()).hexdigest());release.write_text(json.dumps(metadata,indent=2))
        emit('Changed: metadata schema and application; application restarted; database/host not restarted; backup='+str(backup))
    safe_directory=ROOT/'reports/user-admin-live-files'
    ...
    if operation=='inspect':
        ...
        token,_=new_session(admin[0])
        try:
            for path in ('/interactive-map/','/admin/users','/api/admin/users','/interactive-map/users.js','/admin/hexes','/api/hexes','/api/start/options'):
            ...
            with connect() as conn:conn.execute('DELETE FROM user_sessions WHERE token_hash=%s',(token_hash(token),))
    if operation=='inspect':
        names=('app/backend/main.py','app/backend/admin_users.py','app/frontend/app.js','app/frontend/index.html','app/frontend/users.html','app/frontend/users.js','data/migrations/004_user_admin.sql','data/migrations/005_user_deletion_audit.sql')
    ...
    output=ROOT/'reports/user-admin-source.json'
