...
param([string]$ResourceId = '',
      ...
      [ValidateSet('inspect', 'validate', 'deploy', 'timer-check')][string]$Operation = 'inspect')
...
try {
    ...
    if ($ScriptName -eq 'GameAccessRunner.py') {
        & scp.exe @opts "$root/scripts/automation/GameAccessIdentity.py" 'avalon@10.210.52.128:/home/avalon/rhisseth.ru/scripts/automation/GameAccessIdentity.py'
        if ($LASTEXITCODE -ne 0) { throw 'Identity helper transfer failed' }
        & scp.exe @opts "$root/scripts/automation/GameAccessVps.py" 'avalon@10.210.52.128:/home/avalon/rhisseth.ru/scripts/automation/GameAccessVps.py'
    ...
    if ($ScriptName -in @('Deploy-RhissethFromRunner.py','GameAccessRunner.py')) { $command += " $Operation" }
